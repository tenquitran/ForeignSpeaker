	  ...
	  m_clientWidth(clientWidth), m_clientHeight(clientHeight)
{
	...
	LoadString(hInstance, IDS_APP_TITLE, m_titleBarStr, MaxLoadString);
	LoadString(hInstance, IDC_FOREIGNSPEAKER, m_windowClassName, MaxLoadString);
...
ATOM MainWindow::registerWndClass()
{
	...
	wcex.hIcon = LoadIcon(m_hAppInstance, MAKEINTRESOURCE(IDI_FOREIGNSPEAKER));
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCE(IDC_FOREIGNSPEAKER);
...
{
	HACCEL hAccelTable = LoadAccelerators(m_hAppInstance, MAKEINTRESOURCE(IDC_FOREIGNSPEAKER));
